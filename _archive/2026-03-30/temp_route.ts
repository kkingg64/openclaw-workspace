// API Route for Meal Generation - Anthropic SDK Compatible
import { NextRequest, NextResponse } from 'next/server';

const DEFAULT_MINIMAX_ANTHROPIC_URL = 'https://api.minimax.io/anthropic';

export async function POST(request: NextRequest) {
  const MINIMAX_BASE_URL = process.env.MINIMAX_BASE_URL || DEFAULT_MINIMAX_ANTHROPIC_URL;
  ...
  try {
    ...
    const prompt = `I need 3 Chinese recipes. Output ONLY valid JSON, NO explanation.
...
IMPORTANT: Replace the example recipes with REAL Chinese recipes. Use real ingredient names. Output starts with { and ends with }.`;
    ...
    const apiUrl = `${MINIMAX_BASE_URL}/v1/messages`;
    ...
    console.log('Raw Anthropic response (first 300 chars):', content.substring(0, 300));
